...
class CalendarController {
    ...
    private generateConfirmation = (event: CalendarEvent): string => {
        ...
        const parts = [
            `${this.getActionVerb(event)}: "${event.summary}" for ${formatDateTime(startTime)}`
        ];
    ...
    checkAuthStatus = async (req: Request, res: Response) => {
        try {
            ...
            // First check if auth_session cookie exists - this is our primary auth indicator
            if (req.cookies.auth_session === 'true') {
                console.log('Auth cookie exists, skipping service check');
                
                // Get the token expiry from cookie if it exists
                let expiryDate = req.cookies.auth_expiry ? 
                    parseInt(req.cookies.auth_expiry) : 
                    Date.now() + (60 * 60 * 1000); // Default 1 hour
                
                // Get user profile if possible
                let userProfile = null;
                try {
                    userProfile = await googleCalendarService.getUserProfile();
                } catch (profileError) {
                    console.log('Could not fetch profile, but auth is valid:', profileError);
                }
            ...
            // Fallback to service check if no cookie
            const isAuthenticated = await googleCalendarService.isUserAuthenticated();
            console.log('Service auth status result:', isAuthenticated);
            
            // If service says authenticated but no cookie, set it
            if (isAuthenticated && !req.cookies.auth_session) {
            ...
            let userProfile = null;
            let expiryDate = Date.now() + (60 * 60 * 1000); // Default 1 hour
            ...
            if (isAuthenticated) {
                try {
                    ...
                    console.log('Error getting profile or tokens:', e);
                }
            }
